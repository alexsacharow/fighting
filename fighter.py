...
class Fighter:
    ...
    def __init__(self, player, x, y, flip, data, sprite_sheet, animation_frames, attack_sound, hit_sound):
        ...
        self.image = self.animation_list[self.action][
            self.frame_index
        ]  # Assign first frame of fighter's current action animation
    ...
    def update(self):
        ...
        ):  # If next animation index is out of range
        ...
        if self.attack_cooldown == 0 and pygame.time.get_ticks() - self.last_attack_time > ATTACK_DELAY:
            ...
            if self.flip:
                attacking_rect = pygame.Rect(
                    self.rect.left - attack_width, self.rect.y, attack_width, attack_height
                )
            else:
    ...
    def draw(self, surface):
        ...
        if self.action == 5:  # Hit animation
            surface.blit(
                img,
                (
                    self.rect.x - (self.hit_offset[0] * self.image_scale),
                    self.rect.y - (self.hit_offset[1] * self.image_scale),
                ),
            )
            ...
            surface.blit(
                img,
                (
                    self.rect.x - (self.death_offset[0] * self.image_scale),
                    self.rect.y - (self.death_offset[1] * self.image_scale),
                ),
            )
            ...
            surface.blit(
                img,
                (
                    self.rect.x - (self.custom_offset[0] * self.image_scale),
                    self.rect.y - (self.custom_offset[1] * self.image_scale),
                ),
            )
        else:
